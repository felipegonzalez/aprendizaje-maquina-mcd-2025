---
title: "Tarea 5 - redes neuronales simples, regularización y descenso estocástico"
format: html
---
...
# en el caso de keras, y en contraste con el funcionamiento
# usual, los pipes |> actualizan el objeto original (ejemplo_mod)
ejemplo_mod |> 
   layer_dense(units = 2, 
...
**Pregunta 2**: En la función objetivo que intenta minimizar este ajuste,
cuáles son los parámetros que tienen penalización L2? (puedes ver
las notas de clase y la [documentación de keras](https://keras.io/api/layers/regularizers/))

Graficamos el modelo resultante para ver que obtuvimos resultados similares a los de la clase:
...
**Pregunta 3** Examina esta última salida. En esta lista: 
...
**Pregunta 4**: prueba este mismo ejemplo usando un tasa de aprendizaje
...
g <- ggplot(tibble(x, f), aes(x, f)) + geom_line(col ="red")
...
```
**Pregunta 5**: En este punto, ¿es buena idea intentar usar regularización L2 más grande para
mejorar el ajuste? Explica tu respuesta.

**Pregunta 6**: ¿crees que incrementando el número de iteraciones / afinando 
...
Repetimos ahora con un número más grande de unidades, y sin regularización. 
Usaremos descenso en gradiente estocástico con momento (es decir, la actualización se
hace un una dirección promedio entre la dirección del minilote y la dirección
promedio actual: puedes ver el apéndice en las notas). Usamos un minilote de tamaño 20. 

Recuerda:

- Monitorear el valor del "loss", que indica si el optimizador está
reduciendo la función objetivo
- Puedes poner verbose = 0 más adelante para no tener que borrar la salida larga
de las épocas
- Recuerda que cada corrida, si quieres empezar con una nueva red, tienes
que correr la función `construye_modelo`. De otra forma, las iteraciones que haces
continúan a partir del ajuste anterior.
...
**Pregunta 7**: ¿Qué tan bueno es el resultado que obtuviste? ¿Qué pasa si pones una
tasa de aprendizaje muy alta o muy baja?

**Pregunta 8**: Intenta usar el mismo algoritmo pero con descenso en gradiente usual (sustituye batch_size arriba por el tamaño
