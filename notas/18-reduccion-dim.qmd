...
El primer contexto particular en que estudiamos estas técnicas es el de los
sistemas de recomendación con factores latentes. Los datos de vistas/compras
de usuarios de películas/artículos son de dimensión alta (hay muchas películas
o artículos). Veremos también cómo se utiliza en otros contextos más comunes
de datos mendianos, como construcción de índices de marginación o desarrollo. Otro contexto que veremos será el de procesamiento de lenguaje
natural, que igualmente se presenta naturalmente en un espacio de dimensión
alta (por ejemplo, el número de palabras con las que se puede continuar una frase).


Comenzaremos con la **descomposición en valores singulares**, que es una de las 
técnicas más útiles, se construye con proyecciones lineales de los datos originales,
y es la base de métodos como componentes princiales.
...
- $u$ y $v$ representan una dimensión (dimensión latente o componente) de estos datos.
...
u <- rnorm(15, 0, 1)
u <- normalizar(u)
personas <- tibble(persona = 1:15, u = u)
head(personas)
```
...
en la matriz $R$. Podríamos repetir buscando una aproximación igual a la que acabomos  de hacer para la matriz $X$, o podríamos hacer distintos tipos de análisis.
...
sigma_df <- tibble(dim = c('1','2'), sigma = c(100,70))
```


```{r}
df_dim <- personas_larga |> 
  full_join(peliculas_larga, by = "dim", relationship = "many-to-many") |>
  left_join(sigma_df, by = "dim") |>
  mutate(afinidad = sigma * u * v)
df_agg <- df_dim |> group_by(persona, pelicula) |>
  summarise(afinidad = round(sum(afinidad),2), .groups = 'drop')
...
$$Xv = \sigma u$$ {#eq-vector-propio-derecho}


$$u^t X= \sigma v^t.$$ {#eq-vector-propio-izquierdo}
...
::: callout-note
# Vectores propios derecho e izquierdo

A un par de vectores $(u,v)$ que cumplen esta propiedad les llamamos 
**vector propio izquierdo** ($u$) y **vector propio derecho** ($v$), con **valor singular** asociado $\sigma$.
...
- La demostración es fácil pues aplicando $X^t$ a ambos lados de (@eq-vector-propio-derecho), obtenemos
$X^t X v= \sigma X^t u$, que implica $(X^t X) v= \sigma (u^tX)^t = \sigma^2 v$. Podemos
hacer lo mismo para (@eq-vector-propio-izquierdo).
...
## Interpetación de vectores propios
...
::: callout-note
# Resumen de vectores propios

Si $u$ y $v$ son vectores propios derecho e izquierdo de $X$, 
con valor singular $\sigma$, entonces
...
- Si escogemos el par $u,v$ de vectores propios con el valor singular $\sigma$ más grande posible, entonces el vector $v$ es la dirección tal que las proyecciones de los reglones de $X$ sobre $v$ son lo más cercanas posibles a los renglones originales $x_i$
:::
...
- Frecuentemente el teorema de aproximación óptima (teorema de Ekhart-Young) 
...
```{r}
proyecciones <- tibble(dim_1 = d[1]*u$x_1, dim_2 = d[2]*u$x_2,
                          selec = datos$selec) 
...
if(TRUE){
  ...
  set.seed(281)
  svd_parcial <- irlba(evals_mat, 6, maxit = 2000)
}
```
...
ggplot(scores_w, aes(x=Comp.1, y= Comp.2, label=Distillery)) + 
  ...
  xlab('Fruity/Floral vs. Smoky/Medicinal') +
