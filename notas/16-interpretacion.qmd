...
receta_casas <- recipe(precio_miles ~ 
  ...
  step_rm(condicion_venta, skip = TRUE) |> 
...
predictor_bo <- Predictor$new(model = bosque_ajustado, data = casas_pr, 
...
vars_usadas <- extract_preprocessor(bosque_ajustado) |> pluck("var_info") |>
  ...
  pull(variable)
imp_bosque <- FeatureImp$new(predictor_bo, loss = "mape",  
...
```{r, fig.width =6, fig.height= 12, cache = TRUE, eval = FALSE}
set.seed(12)
