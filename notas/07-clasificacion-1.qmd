...
y_ent <- (diabetes_ent_s$type == "Yes") |> matrix(ncol = 1)
x_prueba <- diabetes_pr_s |> select(-type, -id) |> as.matrix()
y_prueba <- (diabetes_pr_s$type == 'Yes') |> matrix(ncol = 1)
...
**no es necesario si usamos regularización**, que generalmente es el caso. En aprendizaje
automático, como usualmente existe regularización (en la función objetivo o el método de ajuste,
por ejemplo), típicamente no se fija una clase de referencia para problemas de 3 o más clases.
...
save_model(modelo_ropa, "cache/red_ropa_1.keras")
}

modelo_ropa <- load_model("cache/red_ropa_1.keras")
scores <- modelo_ropa |> evaluate(
  x_prueba, y_prueba, verbose = 0
)
cat('Test pérdida:', scores[[2]], '\n')
cat('Test clasificación correcta:', scores[[1]], '\n')
