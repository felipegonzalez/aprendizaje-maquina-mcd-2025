...
receta <- 
  ...
  step_other(country, threshold = tune("threshold_country"), other = "other_c") |>
  step_other(agent, threshold = tune("threshold_agent"), other = "other_a") |>
...
Esta parte tienes que ajustarla según el número de cores
de tu computadora:
...
params <- parameters(learn_rate(range = c(-5, -1), log10_trans()),
                     mtry(range = c(5, 30)),
                     tree_depth(range = c(3, 10)),
                     trees(range = c(25000, 45000)),
                     threshold_agent= threshold(range = c(0.001, 0.1)),
                     threshold_country = threshold(range = c(0.001, 0.1)),
                     loss_reduction(range = c(-10, -5)),
                     stop_iter(range= c(80, 100)))

#lambda_grid <- grid_random(params, levels = c(learn_rate = 20, mtry = 6, tree_depth = 6, trees = 3,
#                                              threshold = 5), size = 100)
lambda_grid <- grid_random(params, size = 100) #500
